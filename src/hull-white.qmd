...
ここで、右辺第一項は時点 $t$ におけるイールドカーブと瞬間短期金利 $r_t$ の関係を定めている。
連動の強さを表す係数 $\beta(u)=(1-e^{-\alpha u})/(\alpha u)$ は残存期間 $u$ に依存しており、$u \to 0$ で $\beta(u) \to 1$、$u \to \infty$ で $\beta(u) \to 0$ であるから、残存期間が短期であるほど $r_t$ に強く連動し、長期であるほど $r_t$ の変化への反応が鈍くなります。
